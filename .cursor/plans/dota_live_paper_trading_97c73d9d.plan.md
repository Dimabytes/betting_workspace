...
todos:
  - id: model-registry
    content: "Реестр моделей: data/new_model/current/ с model.json, старая уезжает в data/new_model/<name>/; backtest пишет model_name вместо sha; делаем до переобучения"
    status: pending
  - id: xp-level
  ...
  - id: steam-feed
    ...
    status: pending
  - id: state-writer
    content: "state_writer.py: сырые снапшоты в JSONL, по файлу на матч, fsync и докат после падения"
    status: pending
  - id: match-meta
  ...
  - id: discovery
    ...
    status: pending
  - id: model-server
    content: "model_server.py: Booster, вектор FEATURE_COLUMNS, predict_fair; адаптер пары YES/NO в YES-fair"
    status: pending
  - id: paper-gateway
    content: "paper_gateway.py: филлы в StateStore.apply_fill, open_orders() отдаёт живые paper-ордера"
    status: pending
  - id: session
    content: "session.py: обёртка FV под сигнатуру движка, REDUCE_ONLY вне окна и на паузе (нет новых BUY, SELL если есть позиция)"
    status: pending
  - id: tuning
  ...
  - id: shared-extract
    ...
    status: pending
  - id: parquet-convert
    content: "state_to_parquet.py: JSONL матча -> parquet для train/valid; отдельный запуск, не в лайве. winner=null добираем из OpenDota здесь, не в демоне"
    status: pending
  - id: report
  ...
  - id: docker
    ...
    status: pending
  - id: tests-lint
    content: Юнит-тесты (уровень->XP, правило филла, ориентация YES/Dire, REDUCE_ONLY вне окна и на паузе, обёртка FV видна движку) + чистый make lint-all
    status: pending
  - id: docs
...
- **Сырые снапшоты, без parquet в лайве.** Лайв пишет обычный JSONL (без gzip). Parquet собирает отдельный скрипт по требованию.
- **Один матч — один подпроцесс.** Оркестратор следит за подпроцессами.
- **Вне окна модели и на паузе не покупаем.** Модель училась отвечать: «какая будет цена рынка, когда игровые часы дойдут до текущей секунды плюс 300». Это 300 секунд игры, не 300 секунд настенных часов: пауза в них не входит. На паузе состояние игры стоит, а рынок идёт — модель на это не училась, поэтому новых покупок нет. Если позиция уже есть, её продаём. Все ордера разом не снимаем.
...
- **Разработка и запуск на VPS.** Сайдкаров коллектора на ноутбуке нет, второй коллектор локально не поднимаем. Живой демон — Docker Compose на VPS рядом с коллектором, `restart: unless-stopped`, тот же `ARCHIVE_ROOT`. Код лайва правим там же, в checkout, смонтированном в контейнер. На ноутбуке остаётся то, для чего нужны датасеты: пересборка датасета, обучение, бэктест. Первый матч смотрим по логам и Telegram.
...
flowchart LR
  ...
  subgraph sess [Сессия: один подпроцесс на матч]
    feed["steam_feed: GetRealtimeStats 1 Гц"] --> raw["state_writer: JSONL сырых снапшотов"]
    feed --> snap["GameSnapshot: second, nw_adv, xp_adv из уровней, deaths"]
    snap --> model["model_server: Booster, delta"]
    book["CLOB market WS движка"] --> mid["Пара YES/NO → market_p_radiant"]
    mid --> model
    model --> adapter["Обёртка: radiant-fair → YES-fair"]
    adapter --> eng["Engine paper=True"]
    eng --> gw["PaperGateway: филлы в StateStore"]
    gw --> eng
  end
...
### A0. Реестр моделей (делаем первым)

Сейчас модель — один файл `data/new_model/model.txt`, который переобучение затирает. Имени у модели нет, дата обучения нигде не записана, старую версию вернуть нельзя. Бэктест и лайв опознают модель по `sha256` — это работает, но по нему не скажешь ни когда модель обучили, ни на каком контракте.

Делаем это до части A4, чтобы переобученная модель сразу легла в реестр, а нынешняя сохранилась.

**Раскладка.** Каталоги плоские, вложенности на два уровня нет.

```
data/new_model/
  current/                     # актуальная модель, её читают все
    model.txt
    model.json                 # имя и контракт
    split.parquet
    validation_scenarios.csv
  20260814T193005Z/            # прошлая, имя = её же timestamp
    model.txt
    model.json
    ...
```

**Имя** — метка времени обучения в UTC, `%Y%m%dT%H%M%SZ`. Сортируется как строка, годится для имени каталога.

**`model.json`:**

```json
{
  "name": "20260814T193005Z",
  "trained_at": "2026-08-14T19:30:05Z",
  "features": [
    "second",
    "radiant_nw_adv",
    "radiant_xp_adv",
...
`source_lag_seconds` и `poll_interval_seconds` пишем не для справки. Модель с лагом 8 и модель с лагом 2 — разные контракты. Сессия сверяет их со своими константами и отказывается стартовать при расхождении. Метрики берём из `evaluate_validation_metrics`, обучение их и так считает.

**Правки:**

- `src/shared/constants/paths.py:48-51` — `CURRENT_MODEL_DIR = NEW_MODEL_DIR / "current"`, `NEW_MODEL_PATH`, `NEW_MODEL_SPLIT_PATH` и `VALIDATION_SCENARIOS_PATH` переезжают внутрь него, плюс `MODEL_META_PATH`. Читатели трогать не надо: они импортируют константы.
- `src/train_model/train_model.py:171` — перед сохранением: читаем `current/model.json`, переименовываем `current/` в `data/new_model/<name>/`, создаём новый `current/`, пишем `model.txt` и `model.json`. Переименование каталога атомарно, промежуточного состояния нет.
- `src/backtest/run.py:479` — в манифесте `model_sha256` меняем на `model_name` из `model.json`. `--resume` сверяет его так же, как раньше сверял хэш: имя меняется на каждом обучении.
- `src/backtest_viewer/app.py:193` — предупреждение «модель на диске другая» сверяет имена, а не хэши.
- `data/new_model/current/model.json` для нынешней модели пишем руками, один раз. Тогда в коде не нужна ветка «а если json нет».

**Git.** `.gitignore` уже содержит `!data/new_model/`, поэтому подкаталоги попадают в git сами. Одна модель — около 230 КБ (`model.txt` 192 КБ, `split.parquet` 30 КБ, csv 5.5 КБ). Держим все.

### Три изменения контракта Steam

Три изменения. Все три ломают текущую модель, поэтому их делаем вместе и переобучаем один раз.
...
Гейт: если новая модель заметно хуже старой, в лайв не идём. Аблации нет: три изменения включены вместе, одна новая модель против старой. Если просело — разбирать потом, не в этой стори.

## Часть B — архив состояния игры

### B1. `src/live_paper/steam_feed.py`

Следит за одним матчем. Номер сервера уже известен: его передал оркестратор. Опрашивает `GetRealtimeStats` по `server_steam_id` раз в секунду, вычитая время запроса из сна. Отдаёт два потока: сырой payload для записи и `GameSnapshot` для модели.
...
- Конец — `game_state >= 6`.
- `server_steam_id` фид не ищет: его передаёт оркестратор. Без этого номера `GetRealtimeStats` не вызвать.

**Слабое звено: `server_steam_id`.** Его нет в `GetLiveLeagueGames`. Только `GetTopLiveGame` (10 записей на слот `partner` 0..3). Без этого номера снимок не вызвать — для оркестратора это просто «нет совпадения», сессию не стартуем. На следующем круге список Steam будет новый. OpenDota не зовём.
...
Пишет сырой payload как есть, по строке на снапшот, в `data/live_paper/<match_id>/state.jsonl`. Каждая строка — объект с локальным временем приёма, временем запроса и телом ответа. Никакого разбора: разбор — задача конвертера. Gzip в v1 нет: после падения процесса дописать в gzip нельзя без риска испортить файл.

Размер: снапшот 16.6 КБ. Матч на 40 минут — около 40 МБ. Это нормально.

Дозапись после падения: оркестратор перезапускает сессию, та открывает файл на добавление, после каждой строки — flush и fsync. Разрыв виден по прыжку `game_time` и попадает в метадату как `gaps`.

### B3. `src/live_paper/match_meta.py`

`data/live_paper/<match_id>/match.json`, две записи.

При старте: `match_id`, `server_steam_id`, `league_id`, имена команд и какая сторона какая, номер карты, `joined_at_second` (с какой секунды подключились), локальное UTC подключения, оценка UTC горна (`timestamp - game_time`), блок рынка (`condition_id`, `market_slug`, `event_slug`, оба `token_id`, `yes_is_radiant`, `tick_size`, `min_order_size`, `neg_risk`, `grid_series_id`) и блок `model` (`name`, `trained_at` из `model.json` — см. A0 и C2).
...
Демон только пишет JSONL и `match.json`. После матча (или пачкой) два разных запуска:
...
2. **`report.py` / `make live-report`.** Не parquet. Таблица по тем матчам, которые мы **торговали** в paper: филлы, реализованный и нереализованный PnL, аптайм фида, покрытие сигналом, пропуски секунд, `model.name`, итог. Итог разбиваем по имени модели, иначе матчи двух моделей складываются в одно бессмысленное число. Нужен, чтобы смотреть «сколько модель заработала бы», а не чтобы учить.
...
Одна функция: отправка текста в Telegram через Bot API. Токен и чат берём из `TG_BOT_API_TOKEN` и `TG_CHAT_ID` — те же имена, что у `polymarket-collector`. Переменных нет — в Telegram не шлём, в лог пишем что отправить не вышло.

Три точки вызова:

1. Перешли на второй ключ Steam, или 429 уже на втором (B5).
2. Рынок Polymarket есть, а совпадающей игры Steam нет. Это тихая потеря матча, о ней надо знать сразу.
...
**Частота.** Каталог сайдкаров и Steam опрашиваем в одном цикле, раз в 60 секунд. Тридцати секунд не нужно. В том же цикле: `GetLiveLeagueGames` (имена, серия, номер карты) и `GetTopLiveGame` partner 0..3 (`server_steam_id`). Совпадение — это рынок + игра Steam с теми же именами, тем же номером карты и с `server_steam_id`. Нет номера сервера — это не совпадение. OpenDota не зовём.
...
Грузит `data/new_model/current/model.txt` через `lgb.Booster` и рядом `model.json`. Собирает вектор `FEATURE_COLUMNS` из `src/train_model/train_model.py`. `predict_fair(snapshot, market_p_radiant) -> clip(market_p_radiant + delta, 0, 1)`. Это цена Radiant, не цена исхода YES на Polymarket.

На старте сверяет `model.json` со своими константами: список фич, `source_lag_seconds`, `poll_interval_seconds`. Расхождение — сессия не стартует и пишет в Telegram. Это защита от модели, обученной на другом контракте.

Кормим модель свежим снапшотом Steam без дополнительной задержки. После части A2 контракт обучения совпадает с тем, что видит бот. В модель в поле `second` идёт `game_time` снапшота как есть: вычитать лаг второй раз нельзя (обучение уже вычло его из колонки датасета).

**Подмена модели без перезапуска.** Сессия грузит файл один раз, на старте. Один матч — один подпроцесс, значит новую модель подхватит следующий матч, а идущий доиграет на своей. Так и надо: две модели в одном PnL не смешиваются.

Что для этого нужно: `data/new_model` смонтирован томом, а не запечён в образ (см. часть E). Кладём новый каталог `current/` на VPS — следующая сессия берёт новую модель.

Гейт по контракту делает подмену безопасной. Модель на других фичах не запустится, а не даст тихо неверный прогноз. Тогда нужен `docker compose up -d --build`: `FEATURE_COLUMNS` живут в коде.

**Что записываем.** `model.name` из `model.json` — в `match.json` и первой строкой `session.jsonl`. Отчёт из части D группирует PnL по имени модели. Хэши больше нигде не считаем.

### C3. `src/live_paper/paper_gateway.py`

`PaperGateway(ExecutionGateway)`. Хранит ордера, которые движок «поставил». Правило филла: стоящий BUY по цене `p` исполняется, только когда лучший аск **ниже** `p` или последняя сделка ниже `p`. Для SELL зеркально. Касание цены филлом не считается: ордер ещё стоит в очереди.

Движок в paper не видит филлы с биржи (пользовательский сокет не запускается). Позицию он читает из своего `StateStore`, не из гейтвея. Поэтому:

1. Переопределяем `open_orders()` (не `get_open_orders` — такого метода нет). Он обязан вернуть те же живые paper-ордера. Иначе раз в 20 секунд цикл сверки движка получит пустой список и сотрёт ордера, квотер поставит их заново, филлы разъедутся.
2. Филл пишем в `state.apply_fill` и зовём `engine._on_fill`. Тогда скью `gamma` и лимиты `q_max_usdc` / `q_soft_frac` видят позицию.
3. `positions()` у биржевого гейтвея в paper и так пустой; пустой ответ сверку позиции не затирает (`if positions:`). Не заменяем его «фейковым REST».
...
1. Собирает временный config-dir: `config.toml`, `strategy.toml`, `markets.toml`. Числа профиля `dota-map` не в коде, а в `config/dota-map.toml` в репозитории. Сессия копирует его в свой config-dir. Тюнинг стратегии — правка одного файла, без правки Python.
...
3. Патчит `compute_fair_value` и `ExecutionGateway` в модуле `polymaker.engine` до создания `Engine`.
4. Запускает `Engine(cfg, paper=True)` с `PaperGateway`.

**Патчим два имени, и оба — в модуле `polymaker.engine`.**

Движок вызывает `compute_fair_value(micro, flow_z, tick)`, где `micro` — цена исхода YES, а котировки строятся вокруг YES. Модель даёт цену Radiant. Подставить `predict_fair` на это место нельзя: другая сигнатура, и при YES = Dire все котировки перевернутся.

Обёртка закрывает ячейку с последней ценой YES. На каждом снапшоте Steam сессия:

1. Берёт мид YES и мид NO из книг движка.
2. По `yes_is_radiant` складывает их в `market_p_radiant` тем же `normalize_pair_mids`, что бэктест.
3. Зовёт `predict_fair`.
4. Если YES — не Radiant, пишет `1 - цена`. Это и есть то, что вернёт обёртка.
5. Будит `_wake_cid`.

Если пары нет (сумма мидов далеко от 1) или сигнала нет — обёртка вызывает исходный `compute_fair_value`. Тест: подмена в `polymaker.engine` видна движку; подмена в `polymaker.strategy.quoting` — нет; при `yes_is_radiant=False` YES-fair = `1 - radiant`.

```python
import polymaker.engine as pe
pe.compute_fair_value = yes_fair_adapter  # сигнатура (micro, flow_z, tick), НЕ predict_fair
pe.ExecutionGateway = PaperGateway        # до Engine(...): __init__ создаёт гейтвей сам
```

`engine.py:40` делает `from polymaker.strategy.quoting import compute_fair_value`. Такой импорт копирует имя в модуль `polymaker.engine`. Подмена атрибута в `polymaker.strategy.quoting` на движок не подействует.

**Одно правило окна и паузы.** Модель валидна при `0 <= second <= 600` и не на паузе, когда есть свежий снимок. Тогда покупаем и продаём как обычно, FV из модели.

Иначе (до горна, после 10-й минуты, пауза, снимок протух, пара цен сломана): **новых покупок нет**. Если позиция уже есть — продаём её как мейкер. Все ордера разом не снимаем.

Это режим `REDUCE_ONLY`, который в движке уже есть: он не ставит BUY и оставляет SELL по инвентарю. Сессия включает его, подменяя `RegimeMachine.decide` (или выставляя `risk_reduce_only`). Форк не трогаем. Не `HALTED`: тот снимает все котировки. Для цены SELL вне окна обёртка отдаёт обычный микропрайс движка.
...
Главный цикл раз в 60 секунд:

1. Читаем открытые рынки Polymarket (сайдкары) и живые игры Steam.
2. Если рынок и игра совпали и такого `match_id` ещё нет среди подпроцессов — стартуем сессию.
3. Если не совпали — ничего не делаем.
4. Смотрим, что уже запущено, чтобы не поднять тот же матч второй раз.

Совпадение = имена команд + номер карты + `server_steam_id`. Фид в сессии номер не ищет: ему его передают.

Дальше как раньше: остановить сессию по концу матча, перезапуск упавших с backoff, раскладка `data/live_paper/`. Цель `make live-paper`.
...
`daily_loss_kill_usdc` — это **не размер сделки**. Это аварийный стоп по убытку. Дефолт `RiskConfig` — 250, в живом конфиге политического бота бывает 40. Счётчик живёт в памяти процесса и обнуляется на старте движка, поэтому у каждой сессии он свой: общего лимита на все матчи нет. В paper ставим 1000 — при `q_max_usdc = 20` он не сработает никогда и не будет маскировать PnL. Размер симулируемого ордера задаёт `base_size_usdc`, не это поле.
...
| ---------------------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base_size_usdc`       | **5**                                    | Номинал **одного** входа. При цене 0.50 это 10 шаров. Минимум биржи на доте — 5 **шаров**, не 5 долларов, поэтому $5 упирается в этот минимум (см. ниже).                                                                                            |
| `q_max_usdc`           | **20**                                   | Потолок нетто-инвентаря на рынок. 4 полных филла по 5 и добавляющая сторона выключается. Держим то же отношение 1:4, что было при 50/200: инвентарный скью и `q_soft_frac` считаются от `q_max`, и при 200 они за десять минут не включатся ни разу. |
| `daily_loss_kill_usdc` | **1000** (в `config.toml`, не в профиле) | Стоп по убытку. Не размер сделки. При `q_max = 20` максимальный убыток сессии — десятки долларов, поэтому стоп не сработает никогда. Так и надо: он не должен маскировать PnL.                                                                       |

**Счётчик убытка — свой на каждый матч.** `RiskManager` держит `_net_cash` и `_day_start_equity` в памяти (`risk/manager.py:31`), `Engine.run_forever` зовёт `reset_day()` на старте (`engine.py:129`). Один матч — один подпроцесс — свой Engine. Общего счётчика на все матчи нет. «Дневной» стоп на деле посессионный.

**Минимум биржи ломает размер $5.** `_add_layers` (`quoting.py:180`) делит номинал на слои, а потом поднимает каждый ордер до `max(rewards_min_size * reward_size_mult, min_order_size)` = 5 шаров. Считаем при `layers = 2`:

| Цена | Номинал | Шаров всего | На слой | Что реально встанет                    |
| ---- | ------- | ----------- | ------- | -------------------------------------- |
| 0.50 | $5      | 10.0        | 5.00    | 2 × 5 шаров = $5. Совпало.             |
| 0.70 | $5      | 7.14        | 3.57    | поднято до 5: 2 × 5 = $7. Перебор 40%. |
| 0.90 | $5      | 5.56        | 2.78    | поднято до 5: 2 × 5 = $9. Перебор 80%. |

Поэтому на доте `layers = 1`. Тогда при 0.90 встаёт один ордер на 5.56 шара = ровно $5. Отдельно: когда размер режется (TRENDING даёт 0.5, токсичность и инвентарь — ещё меньше) и остаётся меньше 5 шаров, ордер не ставится вообще. При $5 это цена контракта, не баг. Хотим рабочие слои — нужен `base_size_usdc` от 20.

С бэктестом PnL всё равно не сравниваем: у мейкера другой механизм входа.
...
| `flow_ewma_halflife_s` | 30 (дефолт 120) | Период полураспада EWMA знака сделок. **Полураспад** — за сколько секунд вес старого наблюдения падает вдвое: `вес = 0.5 ** (dt / halflife)` (`estimators.py:41`). При 30 с сделка 30-секундной давности весит половину, минутной — четверть. Больше число — поток помнит дольше. На политике 120 с: рынок вялый. На карте 40 минут 120 с — почти вся память. |

**Спред и скью**

**Скью** — сдвиг обеих котировок из-за нашей позиции. Формула в `quoting.py:79`: `skew = gamma · σ_short · u`, где `u` — нетто-инвентарь в долях `q_max` (от −1 до +1), `σ_short` — короткая волатильность. Дальше `r = FV − skew`: держим YES — обе котировки едут вниз, YES докупать дорого, NO дёшево. Так позиция разгружается сама.

| Ключ              | Дота             | Что делает                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gamma`           | 3.0 (дефолт 0.5) | Сила скью. **0.6 не годится по единицам.** `σ_short` — это корень из EWMA квадратов изменений FV между пересчётами (`estimators.py:88`). У нас пересчёт раз в секунду, FV за секунду ходит на ~0.002-0.005. Значит при `gamma = 0.6` и полном инвентаре скью = 0.6 · 0.003 = 0.002, то есть пятая часть тика: округление его съедает, инвентарь на котировки не влияет вообще. Чтобы при `u = 1` сдвиг был около тика, нужно `gamma ≈ tick / σ_short ≈ 0.01 / 0.003 ≈ 3`. Число оценочное. Пишем `vol_short` в `session.jsonl` на первом матче и пересчитываем по замеру. |
...
**Вола** — волатильность, мера того, как сильно цена дёргается. Считается так: на каждом пересчёте берём изменение FV, возводим в квадрат, гоним через EWMA, берём корень (`estimators.py:77-92`). Единица — доли цены за один пересчёт, у нас за секунду. Два горизонта, короткий и длинный, отличаются только полураспадом.

Важно: после монкейпатча FV даёт наша модель, а не книга. Значит вола меряет дрожание модели, а не рынка. Это же число идёт в спред (`c_vol`), в скью (`gamma`) и в порог TRENDING.

| Ключ                   | Дота             | Что делает                                                                                                                           |
| ---------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `vol_short_halflife_s` | 10               | Короткая вола. Идёт в спред и в скью. 10 с — «что было только что».                                                                  |
...
| Ключ               | Дота             | Что делает                                                                                                                                                                                                                                                                                                                                                            |
| ------------------ | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `q_soft_frac`      | 0.6              | Доля `q_max`, после которой перестаём докупать в ту же сторону. При 0.6 и q_max=20 — после ~$12 нетто в YES новые BUY YES не ставим, BUY NO ещё можно.                                                                                                                                                                                                                |
| `layers`           | **1** (дефолт 2) | Сколько ордеров на сторону. При `base_size_usdc = 5` два слоя не делятся: минимум биржи 5 шаров поднимает каждый слой обратно до 5 и удваивает реальный номинал (таблица выше). Один ордер даёт ровно тот размер, который мы задали.                                                                                                                                  |
| `layer_step_ticks` | 2                | Шаг между слоями: второй ордер на 2 тика ниже первого. Шаг в 1 тик бессмысленен — оба ордера стоят вплотную и уходят в одном свипе, это то же самое, что один ордер вдвое больше. Шаг 2 значит, что второй слой берут только на более глубоком движении и по лучшей цене. **При `layers = 1` ключ не работает.** Оставляем на случай, если поднимем `base_size_usdc`. |
...
| `event_jump_ticks` | 15 (дефолт 8)  | Скачок FV на N тиков = EVENT. Считается **между двумя соседними пересчётами** (`regime.py:52`), а не за окно. У нас пересчёт раз в секунду, за секунду FV ходит на доли тика, поэтому порог почти никогда не сработает — ни 15, ни дефолтный 8. Реальный вход в EVENT остаётся один: свип по книге. |
...
| `exit_urgency_s`      | 300 (дефолт 900) | **Ключ мёртвый.** Движок нигде не считает `yes_exit_urgency`, в `QuoteInputs` всегда приходит 0.0 (`quoting.py:57`, вызов в `engine.py:448`). Проверено grep по всему форку: имя встречается только в `config.py` и в тестах. SELL стоит на `FV + δ` и к касанию не идёт, пока не включится REDUCE_ONLY (там жёстко 0.5). Форк не правим, значит выход из позиции в хвосте пассивный. Смотрим по первым матчам, сколько инвентаря остаётся к концу. |
...
services:
  live-paper:
    ...
    volumes:
      ...
      # модель, код и настройки — томами, не из образа: git pull меняет их
      # без пересборки, следующая сессия стартует уже на новых
...
Падение: Docker поднимает оркестратор заново. Сессии — подпроцессы, умирают вместе с родителем. После рестарта оркестратор читает сайдкары, находит ещё живые матчи, открывает `state.jsonl` на добавление. Разрыв виден по прыжку `game_time`.

### E1. Что нужно перезапускать, а что нет

Разрабатываем прямо на VPS: правим файлы в checkout, который смонтирован в контейнер. Архив состояния игры должен идти всегда, а стратегию хочется править часто. Разносить это на два демона не нужно: сессия — отдельный процесс на каждый матч, а Python читает `.py` при старте процесса. Значит правка подхватывается сама, со следующего матча.

| Что поменяли                                | Что делать                     | Идущие матчи                     |
| ------------------------------------------- | ------------------------------ | -------------------------------- |
| Числа стратегии в `config/dota-map.toml`    | ничего, сохранить файл         | доигрывают на старых числах      |
| Код сессии, стратегии, модели, фида         | ничего, сохранить файл         | доигрывают на старом коде        |
| `current/` модель                           | положить каталог на место      | доигрывают на старой модели      |
| Код оркестратора или дискавери              | `docker compose restart`       | умирают, дозапись после рестарта |
| Зависимости, `pyproject.toml`, `Dockerfile` | `docker compose up -d --build` | умирают, дозапись после рестарта |

Первые три строки — вообще без команд, потому что `src`, `config` и `data/new_model` смонтированы томами. Сохранил файл — следующая сессия видит новый.

Модель — единственное, что рождается не на VPS: датасеты и обучение лежат на ноутбуке. Значит `current/` приезжает либо `git pull` (модель в git), либо `scp`. Если перенесём обучение на VPS, git не нужен и здесь.

**Торговля не должна ронять запись.** Внутри сессии архив и движок — не равные части. Фид и `state_writer` крутятся всегда, а весь торговый блок обёрнут в `try/except`: падение движка, модели или гейтвея гасит котировки, пишет Telegram и оставляет запись матча идти дальше. Это дешевле второго демона и закрывает ту же дыру.

```python
# ponytail: торговля изолирована try/except внутри сессии, отдельного демона
# нет. Потолок — общая память и общий GIL с фидом. Если торговый блок начнёт
# тормозить опрос Steam, выносить его в отдельный процесс с чтением jsonl.
```

Риск монтирования исходников: правка посреди старта сессии даст полудописанный файл. Сессия упадёт на импорте, оркестратор поднимет её заново и допишет архив. Ловим по логу: на старте сессия пишет строку с именем модели и текущим коммитом. Правим между матчами.
...
Локально остаётся то, чему нужны датасеты: пересборка датасета, обучение, бэктест. Живой фид, paper и правка их кода — на VPS. Тесты и линт гоняем там, где правим.

## Проверка

- Юнит-тесты и `make lint-all` — локально. Строгий basedpyright на `src/live_paper/`. Проверки датасета из A1 — в шаге подготовки.
- Отдельного сухого прогона нет. Сервис на VPS сам подхватывает первый живой матч. Смотрим логи и Telegram.
- На первом матче три вещи: секунды не пропадают, YES совпадает со стороной Radiant, на паузе нет новых покупок.
...
- Gzip архива состояния.
- Сухой прогон без живого матча.
- Сверка чисел Steam и STRATZ на одном матче до лайва.
