...
todos:
  ...
  - id: docker
    ...
    status: pending
  - id: tests-lint
    content: Юнит-тесты (уровень->XP, правило филла, ориентация, гейт окна, пауза, патч FV виден движку) + чистый make lint-all
    status: pending
  - id: docs
...
```json
{
  ...
  "features": ["second", "radiant_nw_adv", "radiant_xp_adv", "deaths_radiant", "deaths_dire", "market_p_radiant"],
...
Грузит `data/new_model/current/model.txt` через `lgb.Booster` и рядом `model.json`. Собирает вектор `FEATURE_COLUMNS` из `src/train_model/train_model.py`. `predict_fair(snapshot, market_p_radiant) -> clip(market_p_radiant + delta, 0, 1)`.
...
3. Патчит `compute_fair_value` до создания `Engine`.
...
pe.compute_fair_value = predict_fair   # НЕ polymaker.strategy.quoting
pe.ExecutionGateway = PaperGateway     # до Engine(...): __init__ создаёт гейтвей сам
```

`engine.py:40` делает `from polymaker.strategy.quoting import compute_fair_value`. Такой импорт копирует имя в модуль `polymaker.engine`. Подмена атрибута в `polymaker.strategy.quoting` на движок не подействует: он уже держит свою ссылку. Тест на это — обязательный, иначе патч молча не сработает и мы получим микропрайс вместо модели.
