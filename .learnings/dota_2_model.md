...
- Most unmatched GRID games are absent from the Open Access catalog. Do not widen the 1800-second guard or add name matching. TI maps are not in GRID; `04_fetch_grid_starts.py` falls back to `data/live_paper/<match_id>/state.jsonl`: first tick with `game_state == 4` and `game_time <= -80`. That tick is **not** map load: the daemon joins mid-spawn, so map-load unix is `start_timestamp + timestamp - (game_time + 90)`. Measured on 30 fallback matches: `game_time` is −89 (25), −88 (3), −90 (2), so writing the raw tick unix put the horn 1s late (max 2s). Same parquet columns (`grid_game_started_at`, `grid_duration_seconds` from OpenDota duration). Do not read `horn_at_utc` (draft/lobby before 2026-08-21). GRID hit wins; no source column. Fallback needs the match already in `opendota_links` (Explorer, not GRID).
...
- `market_p_radiant` and `market_radiant_prior` are both features (the prior is the second-strongest, 18.7% of gain on `20260822T111856Z`). The prior is also dataset-gate and audit data, but it is never `init_score`.
- STRATZ `stats.networthPerMinute[k]` is the player's net worth at second `60 * k`. The team lead array is one longer and offset: `sum(radiant npm[k]) - sum(dire npm[k]) == radiantNetworthLeads[k + 1]`. Train reads npm at `T >= 0` and playback gold at `T = -60` (no npm index). Validation reads playback gold and checks `playback_nw == npm[second // 60]` at every non-negative minute; minute `-60` checks `leads[0]` only. `build_top_player_features` takes one `list[int]` of per-player net worths per side; there is no player dataclass and no tie-break, because all three top-1 features (`top1_nw_adv`, the two ratios) are `max`/`sum` and so order-invariant. `GameSnapshot` and `ExactSecondState` hold `top: TopPlayerFeatures`; parquet TypedDicts stay flat.
- `FEATURE_COLUMNS` is 12 names in this order: `second`, `radiant_nw_adv`, `radiant_nw`, `dire_nw`, `radiant_xp_adv`, `deaths_radiant`, `deaths_dire`, `top1_nw_adv`, `radiant_top1_nw_ratio`, `dire_top1_nw_ratio`, `market_radiant_prior`, `market_p_radiant`. `model_server` compares `model.json` `features` to that list by ordered equality, so a model trained on a different list cannot load live. Live `_build_feature_values` maps the eight team/market fields from `GameSnapshot` and the three top-player names from `snapshot.top`. Train `radiant_nw`/`dire_nw` sum `networthPerMinute` per side; validation stores the playback team totals already used for `radiant_nw_adv`; live uses Steam team `net_worth`. `radiant_nw_adv` stays on the snapshot for the 30s velocity gate. Absolute-gold `20260819T151315Z` vs buildings-adv `20260819T141947Z` s2-join (413 matches): +$43.31 / +1.31¢ vs +$39.80 / +1.16¢. Buildings `20260819T141947Z` kept vs 13-feat `20260817T212849Z` (+$39.80 / +1.16¢ vs +$39.07 / +1.11¢); note in `dota_2_model/docs/experiments/buildings-adv-features.md`. Scale-free experiment `20260817T191515Z` lost on s2 buy_300s vs kept 13-feat `20260817T173239Z` (1.01¢ [0.06, 1.98] vs 1.43¢ [0.60, 2.26]); it sits in `data/new_model/20260817T191515Z/`.
- Train on seconds `-60, 0, 60, ..., 540` with game at `T` and market at `T+2`. Validation rows are 1 Hz for decision seconds `-58..899` with game at `T-2` and market at `T`; evaluate the model on `-58..599`. Feature `second` on valid/backtest is the GRID clock (`T-2`); the parquet row key stays decision `T`. Live still gates `second < 0` as PRE_HORN.
...
- 1Hz books are local files in `data/raw/telonex/polymarket/`. The Telonex API subscription is dead; do not call it. New days come from polymarket-collector parquet on the VPS (`sun:/var/lib/polymarket-dota-archive/parquet/`). Before a collect that should reach the current date, run `uv run python scripts/sync_collector_parquet.py` first (rsync `--ignore-existing` into the Telonex tree, skips today UTC). Live-paper match archives: `uv run python scripts/sync_live_paper.py` (rsync `sun:/root/work/dota_2_model/data/live_paper/`, excludes `wallet/`, updates existing match files, does not `--delete`). Both share `scripts/rsync_pull.py`; its `parse_argv` returns a frozen `RsyncArgs` and the remote flag is `--remote` in both scripts (`--remote-parquet` is gone). Freeze `make collect AS_OF=` to that run's book `max` day (`T23:59:59Z`). `make prepare` skips up to `MAX_VALIDATION_HARD_MISSES` (20) validation matches with a missing book (collector day holes) and logs them; more than 20, or an invalid cache / worker crash, still abort. Market-seconds `CACHE_PARAMETERS` includes `MODEL_START_SECOND`; changing the window hashes a new `v*` directory so old horn-only caches read as missing and rebuild, not as invalid (invalid still aborts). `check_market_seconds_cache` does **not** validate the horn: a moved `grid_game_started_at` leaves the same second range and identity, so the stale cache reads as `valid` and is reused silently, and `build_market_data.py` only builds `missing` (there is no `--force`). After any change to `grid_game_windows.parquet`, diff the windows and delete the affected `market_seconds/v*/match_id=<id>.parquet` by hand, or `make prepare` rebuilds nothing. Full step-by-step: `dota_2_model/docs/runbooks/rebuild-after-fallback-horn-fix.md`.
...
- Frozen entry-fair SELL (join-ask ≥ first-fill fair − 1¢ until unwind) lost to rolling on s2-join, same model `20260820T133559Z`, 413 matches: +$5.99 vs +$46.87 pre-rebate. Hold got shorter, not longer — frozen-minus-1¢ sells early when rolling fair has risen. Do not re-add `--sell-fair`. Note in `dota_2_model/docs/experiments/entry-fair-sell-gate.md`; run dirs `validation_s2_delta01_cut540_nw350_entry-fair-m1c` vs `…_exit10s`.
- Train `weight=|label|` plus gated early-stop lost on s2-join, model `20260821T174551Z` vs unweighted `20260820T133559Z`, 413 matches: +$12.72 vs +$46.87 pre-rebate, buy 300s 0.36¢ (CI crosses 0) vs 1.68¢. More fills, worse edge. Do not re-add train weights. Note in `dota_2_model/docs/experiments/abs-label-weight.md`; run dir `validation_s2_delta01_cut540_nw350_abs_label_weight`.
- Gated early-stop alone (`|label| >= 1¢` on the val fit, unweighted train) was a wash on s2-join, model `20260821T182710Z` vs `20260820T133559Z`, 413 matches: +$45.88 vs +$46.87 pre-rebate, buy 300s 1.43¢ vs 1.68¢. Do not keep the val gate. Note in `dota_2_model/docs/experiments/gated-early-stop.md`; run dir `validation_s2_delta01_cut540_nw350_gated_early_stop`. `current/` is `20260820T133559Z` again.
- Retrain `20260822T111856Z` (train rows rebuilt from minute leads plus `networthPerMinute`) is a wash, not an upgrade. Compare models **paired on the shared match set**, never by the summary line: the run covered 443 matches while the `20260820T133559Z` baseline covered 413, and the whole headline gain (+$1107.60 vs +$887.90 pre-rebate) is the 30 extra matches (+$259.23). On the shared 413 it is +$848.37 vs +$887.90, paired diff −$0.096/match (t = −0.52), buy 300s 1.744¢ [1.086, 2.402] vs 1.787¢ [1.125, 2.449]. Feature-importance profiles match to a tenth of a percent. Reason: `split.parquet` train ids are byte-identical (2042 matches); every new day lands after `VALIDATION_START_TIME` and so only enters validation. Retraining alone cannot improve the model — moving the cutoff can. `mae_gain` in `model.json` is not comparable across runs either (different `rows`). Run dir `validation_b0_s2_delta01_cut540_nw350_20260822T111856Z`.
...
- `ModelServer.predict_fair(snapshot, market_p_radiant)` returns the RADIANT
  fair price `clip(market_p_radiant + delta, 0, 1)`: one float64 row `(1, 11)` in
...
Do not reintroduce SessionSupervisor/Popen; details in `.learnings/dota_2_model-historical.md`.

Remaining `session_*` helpers (types, config, binding, journal, engine seams,
quoting) are still used.

- Modules: `session_types`, `session_config`, `session_binding`,
  `session_journal`, `session_engine`, `session_quoting`. Names shared across
  them are public (no leading underscore).
- Test-seam: `monkeypatch.setattr` must target the IMPORTER, not the definer.
  `FEED_STALE_SECONDS` is patched on `session_engine`. `notify_in_background`
  is bound at import time — patch the importer.
- Tests: `tests/live_paper_session_fixtures.py` holds builders/fakes; suite is
  `test_live_paper_session{,_config,_binding,_journal,_engine,_quoting}.py`
  plus `test_live_paper_wallet_{store,host}.py` and
  `test_live_paper_engine_seams.py`.
- `shared.utils.strict_json` is the one strict JSON decoder for every
  untrusted document (sidecars, Steam rows, our own provenance):
  `require_object/exact_keys/str/nonempty_str/int/bool/list/str_list/nullable_*`
  raise `StrictJsonError` (a `ValueError`) with a caller-supplied label.
  Exact `type(x) is T`, so `bool` never passes as `int`. Pair nullable reads
  with `require_exact_keys` when an optional field must still be PRESENT as
  an explicit null.
- `archive_paths.FsyncedJsonlWriter` is the one append-only JSONL writer:
  ...
  -> fsync. `StateWriter` and `SessionJournal` are thin wrappers over it
  (fsync test seam is `archive_paths.os.fsync`).
- `SignalReason` is a `StrEnum`, not 15 constants plus a parallel `Literal`.
  A member IS its wire string; compare with `is`/`is not`.
- `cadence.poll_discoveries` uses `asyncio.to_thread`. On Python 3.13
  `asyncio.run` waits at most `THREAD_JOIN_TIMEOUT` (300 s) for the default
  executor, then proceeds — a bounded wait on a call bounded by the httpx
  timeout.
- Durable-final (`match_has_final`): `meta_path.read_bytes()` and
  `json.loads(raw, parse_constant=...)`. Identity-matching: exact-string
  `match_id == requested_id` and a non-null `final`. OSError/UnicodeError/
  JSONDecodeError/RecursionError return false (never raise). A corrupt or
  foreign-id match.json never suppresses a launch or a backoff retry.
...
`current/model.json`, **copy** `split.parquet` aside so the archived snapshot
keeps the split it was trained on, rename `current/` to
...
`panel.parquet` (old market-edge, over the 1 MB add hook), and HTML (Nautilus
`--match-id` dumps / old market-edge Plotly). Viewer is
