...
## Отдельный план перед стадиями 0–7

После завершения десяти шардов выполните
[план реконструкции GRID Net Worth, переобучения и бэктеста](2026-08-30-lol-grid-networth-retrain-plan.md).
Он исправляет Stage 05, переобучает research-модель, запускает полный
бэктест и только после принятия результата публикует production-модель.

Не начинайте стадию 0, пока задача 8 отдельного плана не завершена.
Live-путь читает GRID `NetWorth` как есть. Реконструкция на live не нужна.
