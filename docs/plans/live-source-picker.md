...
todos:
  ...
  - id: "step-3-feed-protocol"
    ...
    status: pending
  - id: "step-4-grid-feed"
    content: "Shared widget parse used by grid_feed.py and watch_grid_live.py; second = clock + clock age - frame delay; one 12s threshold on both model inputs"
    status: pending
...
Самый долгий провал: с 14:10:10 (lag 1.3) до 14:11:09 (lag 60.2). Минуту табло
не присылало часы, пока таблица обновлялась и статус стоял `live`.

За окно 929 с опубликованные часы продвинулись на 928 с. Дрейф −1 с. Часы тикают
секунда в секунду, их публикуют с опозданием, а не замедляют.

**Решение: `second = currentSeconds + (приход таблицы − приход табло) − delay`.**
Только пока `isTicking`. Источник часов один — GRID; наши часы работают
секундомером между двумя приходами, а не вторым источником времени.

Брать `currentSeconds` как пришло нельзя: это ошибка до 60 секунд, и каждый тик
своя.

### Гейт на возраст часов — нужен

Прежний довод «паузу табло пришлёт сразу, поэтому досчитывать безопасно» не
работает. Табло молчит по минуте во время живой игры. Если пауза начнётся в
такой провал, мы досчитаем секунды, которых нет, и не узнаем.

Порог один и тот же на возраст **обоих** входов: таблица и часы, по 12 с.
Старше — тика модели нет. Цена по замеру:

| порог на возраст часов | остаётся тиков |
|---|---|
| 3 с | 49% |
| 5 с | 57% |
| 8 с | 66% |
...
12 с — точка перелома: ниже дорого, выше почти ничего не добавляет. И это то же
число, что для таблицы. Одна константа `GRID_FEED_STALE_SECONDS`, два входа.

Настоящей паузы в этом замере не было (`live` 280 тиков, `paused` только на
границе карт). Как выглядит пауза, ещё не проверено. Порог 12 с ограничивает
ущерб: ошибаемся на 12 секунд, а не на 60.
...
Модели нужны оба: фичи из таблицы, секунда из часов табло. Оба замеряны и оба
уходят в молчание на десятки секунд.

**Правило: тик модели есть, только когда и таблица, и часы моложе 12 с.** Один
порог, одна константа `GRID_FEED_STALE_SECONDS`, два входа. Кадры табло без часов
(киллы, стороны) ни на что не влияют — в фичи они не идут.

| что происходит | правило по двум входам | правило по любому кадру | что правильно |
|---|---|---|---|
...
| таблица шлёт, часы молчат 60 с | сработал | молчит | не котировать |

Третья и четвёртая строки — те, которые нельзя пропускать. В обеих лента
выглядит живой, а один из входов модели гнилой. Вторая строка — почему
отдельного сторожа на тишину сокета не пишем: оба типа кадров едут по одному
соединению, поэтому смерть сокета ловится тем же порогом.

По замеру порог 12 с оставляет 85% тиков. На провале любого из входов уходим в
reduce-only, потом возвращаемся. Это та же механика, что у Steam на 3 с. Крутить
— одну константу.
...
- Порог 12 с на возраст **обоих** входов: последнего кадра таблицы и штампа часов. `GridSocketWatch.note_raw()` в watcher зовётся на каждый кадр — при выносе в модуль считать возраст двух входов, а не любого кадра.
- Карта ≠ `map_number` → skip tick.
- Тесты на payload из [`test_watch_grid_live.py`](../dota_2_model/tests/test_watch_grid_live.py): `second` по формуле выше, XP из `increaseLevel`, и по одному тесту на каждый просроченный вход.
